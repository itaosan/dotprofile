...
AIコーディングエージェント（Codex / Codex CLI）の設定を一元管理するdotfilesリポジトリ。

## プロジェクト構造

```
dotprofile/
├── ai-coding/          # Codex・Codex CLI共通設定
│   ├── install.sh      # メインインストーラ（シンボリックリンク一括作成）
│   ├── AGENTS.md       # 共通エージェント定義（AGENTS.md / AGENTS.mdにリンク）
│   ├── Codex/         # Codex専用設定
│   │   ├── settings.json
│   │   ├── statusline.py
│   │   └── hooks/
...
| リポジトリ内パス | リンク先 |
|---|---|
| `ai-coding/AGENTS.md` | `~/.Codex/AGENTS.md`, `~/.codex/AGENTS.md` |
| `ai-coding/Codex/settings.json` | `~/.Codex/settings.json` |
| `ai-coding/Codex/hooks/` | `~/.Codex/hooks` |
| `ai-coding/commands/` | `~/.Codex/commands`, `~/.codex/prompts` |
| `ai-coding/agents/` | `~/.Codex/agents`, `~/.codex/agents` |
| `ai-coding/skills/` | `~/.Codex/skills` |
